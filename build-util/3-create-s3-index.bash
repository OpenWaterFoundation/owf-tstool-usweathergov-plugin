#!/bin/bash
#
# Create the TSTool US weather.gov plugin product 'index.html' page on Amazon S3,
...
#
# software.openwaterfoundation.org/tstool-usweathergov-plugin/index.html
# software.openwaterfoundation.org/tstool-usweathergov-plugin/index.csv
#
...
#   2018-12-04 16:16:22          0 tstool-usweathergov-plugin/
#   2018-12-04 16:16:37          0 tstool-usweathergov-plugin/1.0.0/
#   2018-12-04 16:17:19   46281975 tstool-usweathergov-plugin/1.0.0/tstool-usweathergov-plugin-win-1.0.0-2201020304.zip
createCatalogFile() {
  ...
  if [ ${errorCode} -ne 0 ]; then
    logError ""
    logError "Error listing TSTool usweathergov plugin download files to create catalog (maybe missing --aws-profile)?"
    exit 1
  fi
  ...
  tmpS3CatalogPath="/tmp/${USER}-tstool-usweathergov-plugin-catalog-ls-installers.txt"
  # Only list installers that match the current download filename specification.
  cat ${tmpS3ListingPath} | grep -E 'tstool-usweathergov-plugin.*.zip' > ${tmpS3CatalogPath}
  logInfo "Available TSTool usweathergov plugin installers:"
...
createIndexHtmlFiles() {
  ...
  indexHtmlTmpFile="/tmp/${USER}-tstool-usweathergov-plugin-index.html"
...
<title>TSTool US weather.gov (usweathergov) Plugin Downloads</title>
</head>' >> ${indexHtmlTmpFile}

echo '
<body>
<h1>Open Water Foundation TSTool US weather.gov (uswethergov) Plugin Software Downloads</h1>
<p>
The TSTool US weather.gov (usweathergov) Plugin software is available for Windows.
...
<li> It is recommended to use the latest plugin version with the latest TSTool version:
     ...
     <li> See the latest <a href="https://software.openwaterfoundation.org/tstool-usweathergov-plugin/latest/doc-user/appendix-install/install/">TSTool US weather.gov Plugin installation documentation</a>
     for installation information (or follow a link below for specific version documentation).</li>
<li>The TSTool usweathergov Plugin requires that TSTool is also installed if not already installed:
    <ul>
...
<ol>
<li> Download a TSTool US weather.gov Plugin Windows installer (zip file) from below.</li>
<li> Follow the latest <a href="https://software.openwaterfoundation.org/tstool-usweathergov-plugin/latest/doc-user/appendix-install/install/">installation instructions</a>
...
<p>Linux versions of the TSTool US weather.gov Plugin are not actively developed.
    ...
    cat "${tmpS3CatalogPath}" | grep "${downloadOs}-" | awk '{ printf "%s %s %s %s\n", $1, $2, $3, $4 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k4,4 | sed 's|-zzz||g' | sed 's|-dev|dev|g'  | awk '
      {
        ...
        downloadFileProduct=""            # The product name (e.g., TSTool US weather.gov Plugin).
        ...
        if ( filenameSpecVersion = 1 ) {
          # Initial filename spec using format:  tstool-usweathergov-plugin/1.0.0.dev/software/tstool-usweathergov-plugin-1.0.0.dev-win-2201020304.zip
          ...
          downloadFileProduct="tstool-usweathergov-plugin"
          downloadFileVersion=downloadFileParts[4]
          downloadFileOs=downloadFileParts[5]
        }
        # Set the URL to the download file.
        downloadFileUrl=sprintf("https://software.openwaterfoundation.org/tstool-usweathergov-plugin/%s/software/%s", downloadFileVersion, downloadFile)
        product = "TSTool US weather.gov Plugin"
        ...
        docDevUrl=sprintf("https://software.openwaterfoundation.org/tstool-usweathergov-plugin/%s/doc-dev",downloadFileVersion)
        ...
        docUserUrl=sprintf("https://software.openwaterfoundation.org/tstool-usweathergov-plugin/%s/doc-user",downloadFileVersion)
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Create the software.openwaterfoundation.org/tstool-usweathergov-plugin/index.html and related files."
...
printVersion() {
  ...
  echoStderr ""
  echoStderr "TSTool US weather.gov (usweathergov) Plugin"
...
uploadIndexFiles() {
  ...
  logInfo "Assuming that TSTool US weather.gov Plugin installer files were previously uploaded to S3."
  ...
  if [ -f "${s3CatalogTxtFileUrl}" ]; then
    ...
    if [ ${errorCode} -ne 0 ]; then
      logError ""
      logError "Error uploading TSTool US weather.gov Plugin catalog file."
      # Not fatal.
    fi
  else
  ...
  #                "Quantity": 1,
  #                "Items": [
  #                    "/tstool-usweathergov-plugin/index*"
  #                ]
  #            },
  ...
  #logInfo "Running: ${awsExe} cloudfront create-invalidation --distribution-id \"${cloudFrontDistributionId}\" --paths '/tstool-usweathergov-plugin/index.html*' --output json --profile \"${awsProfile}\""
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-usweathergov-plugin/index.html' '/tstool-usweathergov-plugin/' '/tstool-usweathergov-plugin' "/tstool-usweathergov-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
    ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-usweathergov-plugin/index.html' '/tstool-usweathergov-plugin/' '/tstool-usweathergov-plugin' "/tstool-usweathergov-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  fi
...
s3FolderUrl="s3://software.openwaterfoundation.org/tstool-usweathergov-plugin"
gpDownloadUrl="https://software.openwaterfoundation.org/tstool-usweathergov-plugin"
...
user=""
tmpS3ListingPath="/tmp/${USER}-tstool-usweathergov-plugin-catalog-ls.txt"
