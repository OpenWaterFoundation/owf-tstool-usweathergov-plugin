...
#   ./tstool/NN/plugins/owf-tstool-usweathergov-plugin/
# - output is to the plugin 'dist' folder with filename like:
#     tstool-usweathergov-plugin-1.0.0-win-2206060102.zip
...
createPluginZipFile() {
  ...
  "${sevenzip}" a -tzip ${zipFile} owf-tstool-usweathergov-plugin
  exitStatus=$?
...
now=$(date +%Y%m%d%H%M)
zipFile="${distFolder}/tstool-usweathergov-plugin-${pluginVersion}-win-${now}.zip"
