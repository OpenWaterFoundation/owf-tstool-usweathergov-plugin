...
getPluginVersion() {
  ...
  # Maven folder structure results in duplicate 'owf-tstool-usweathergov-plugin'?
  # TODO smalers 2022-05-19 need to enable this.
  srcFile="${repoFolder}/owf-tstool-usweathergov-plugin/src/main/java/org/openwaterfoundation/tstool/plugin/usweathergov/PluginMeta.java"  
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Copy the TSTool US weather.gov (usweathergov) plugin installer files to the Amazon S3 static website folder:"
...
uploadInstaller() {
  ...
  installerUploadCount=0
    ...
    if [ "$includeWindows" = "yes" ]; then
      ...
      zipFilePattern="${distFolder}/tstool-usweathergov-plugin-${pluginVersion}-win-*.zip"
      ...
      if [ -f "${latestZipFile}" ]; then
        ...
        if [ ${errorCode} -ne 0 ]; then
          ...
          cloudFrontFile="/tstool-usweathergov-plugin/${pluginVersion}/software/*"
...
logInfo ""
logInfo "${installerUpdateCount} TSTool US weather.gov plugin installers were uploaded to Amazon S3 location:"
logInfo ""
logInfo "  ${s3FolderUrl}"
logInfo "  ${gpDownloadUrl}"
logInfo ""
logInfo "Next steps are to do the following:"
logInfo "  - Visit the following folder to download the TSTool US weather.gov plugin:"
logInfo "      ${gpDownloadUrl}/"
logInfo ""
